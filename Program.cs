...
<body>
    <div class='container'>
        ...
        <h2>Конфигурация веб-приложения</h2>
        <div class='links'>
            <a href='/config' class='btn'>/config - Просмотр конфигурации</a>
            <a href='/api/config' class='btn btn-secondary'>/api/config - API endpoint</a>
        </div>
        <div class='env'>
            Текущая среда: <span id='envName'>Загрузка...</span>
        </div>
    </div>
    <script>
        fetch('/config')
            .then(res => res.json())
            .then(data => {
                document.getElementById('envName').textContent = data.environment;
            })
            .catch(() => {
                document.getElementById('envName').textContent = 'Development';
            });
    </script>
</body>
</html>";
